...
//
//  Created by Xiaobin Zhang on 2021/01/12.
//
...
int list_dir(const std::string& base_dir, std::vector<std::string>& entries)
{
    DIR* dp;
    struct dirent* dirp;
    if ((dp = opendir(base_dir.c_str())) == NULL) {
        std::cerr << "Error(" << errno << ") opening " << base_dir << std::endl;
        return errno;
    }
...
#define CNUM 20
void SORT_tracking(Sort& trackers, std::vector<t_prediction>& detection_results, std::string imagePath, std::vector<std::string> classes, std::vector<std::string> track_classes)
{
    ...
    // update trackers
    trackers.update(detection_frame_data);
    ...
    // read image, draw results and show them
    ...
    for(auto detection_result : detection_results) {
        ...
        cv::rectangle(img, cv::Point(x_min, y_min), cv::Point(x_max, y_max), cv::Scalar(255,255,255), 2, 8, 0);
        cv::putText(img, classes[detection_result.class_index], cv::Point(x_min, y_min-15), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(255, 255, 255), 2, 8, 0);
    }

    // show tracking box in color
    for (auto tb : trackers.m_tracking_output) {
        cv::rectangle(img, tb.box, randColor[tb.id % CNUM], 2, 8, 0);
        cv::putText(img, std::to_string(tb.id), cv::Point(tb.box.x, tb.box.y-15), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(255, 255, 255), 2, 8, 0);
    }
...
void RunInference(Settings* s) {
    ...
    // get track classes labels (optional)
    std::vector<std::string> track_classes;
    if(!s->track_classes_file_name.empty()) {
        std::ifstream track_classesOs(s->track_classes_file_name.c_str());
        std::string track_line;
        while (std::getline(track_classesOs, track_line)) {
            track_classes.emplace_back(track_line);
        }
        int num_track_classes = track_classes.size();
        MNN_PRINT("num_track_classes: %d\n", num_track_classes);
    }
    ...
    // get image list from input path
    std::vector<std::string> images_list;
    list_dir(s->input_imgs_path, images_list);

    std::vector<std::string>::iterator it = images_list.begin();
    while ( it != images_list.end() ) {

        // load input image
        std::string imagePath = s->input_imgs_path + "/" + *it;
        auto inputPath = imagePath.c_str();
        ++it;
        ...
        MNN_PRINT("origin image: path:%s width:%d, height:%d, channel:%d\n", inputPath, image_width, image_height, image_channel);
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    c = getopt_long(argc, argv,
                    "a:b:c:e:hi:k:l:m:n:r:s:t:w:", long_options,
                    &option_index);
    ...
    switch (c) {
      ...
      case 'l':
        ...
        break;
      case 'k':
        s.track_classes_file_name = optarg;
        break;
      case 'm':
        s.model_name = optarg;
        break;
