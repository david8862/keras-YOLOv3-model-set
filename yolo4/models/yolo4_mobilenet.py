...
def yolo4lite_mobilenet_body(inputs, num_anchors, num_classes, alpha=1.0):
    ...
    #feature map 1 head (13 x 13 x (512*alpha) for 416 input)
    x1 = make_yolo_spp_depthwise_separable_head(f1, f1_channel_num//2, block_id_str='14')
    ...
    #feature map 2 head (26 x 26 x (256*alpha) for 416 input)
    x2 = make_yolo_depthwise_separable_head(x2, f2_channel_num//2, block_id_str='15')
    ...
    x3 = make_yolo_depthwise_separable_head(x3, f3_channel_num//2, block_id_str='16')
    y3 = compose(
            Depthwise_Separable_Conv2D_BN_Leaky(f3_channel_num, (3,3), block_id_str='16_3'),
            DarknetConv2D(num_anchors*(num_classes+5), (1,1)))(x3)

    #downsample fpn merge for feature map 3 & 2
    x3_downsample = compose(
            ZeroPadding2D(((1,0),(1,0))),
            Darknet_Depthwise_Separable_Conv2D_BN_Leaky(f2_channel_num//2, (3,3), strides=(2,2), block_id_str='16_4'))(x3)

    x2 = Concatenate()([x3_downsample, x2])

    #feature map 2 output (26 x 26 x (512*alpha) for 416 input)
    #x2, y2 = make_depthwise_separable_last_layers(x2, f2_channel_num//2, num_anchors*(num_classes+5), block_id_str='17')
    x2 = make_yolo_depthwise_separable_head(x2, f2_channel_num//2, block_id_str='17')
    y2 = compose(
            Depthwise_Separable_Conv2D_BN_Leaky(f2_channel_num, (3,3), block_id_str='17_3'),
            DarknetConv2D(num_anchors*(num_classes+5), (1,1)))(x2)

    #downsample fpn merge for feature map 2 & 1
    x2_downsample = compose(
            ZeroPadding2D(((1,0),(1,0))),
            Darknet_Depthwise_Separable_Conv2D_BN_Leaky(f1_channel_num//2, (3,3), strides=(2,2), block_id_str='17_4'))(x2)

    x1 = Concatenate()([x2_downsample, x1])

    #feature map 1 output (13 x 13 x (1024*alpha) for 416 input)
    #x1, y1 = make_depthwise_separable_last_layers(x1, f1_channel_num//2, num_anchors*(num_classes+5), block_id_str='18')
    x1 = make_yolo_depthwise_separable_head(x1, f1_channel_num//2, block_id_str='18')
    y1 = compose(
            Depthwise_Separable_Conv2D_BN_Leaky(f1_channel_num, (3,3), block_id_str='18_3'),
            DarknetConv2D(num_anchors*(num_classes+5), (1,1)))(x1)
